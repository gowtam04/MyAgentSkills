...
## Asset Pipeline
Production `method` per asset family (code-vector | code-raster | procedural | image-gen | greybox | external) and why it fits the GDD art direction.
Naming, resolution, import settings, atlas vs discrete. Where art-generator scripts live.
Greybox-only? If yes, state it. If no: style lock (palette + reference sheet + rules note) and the Asset Manifest below.
...
Split phases into parallel slices wherever they're honestly independent (one per rules module / asset family; contracts and style lock early; hub files owned by one wiring phase). List the waves that can run together and the critical path — the orchestrator runs every ready disjoint slice at once.
...
- export:
- smoke / playtest:
- screenshot (saves PNGs an agent can view):

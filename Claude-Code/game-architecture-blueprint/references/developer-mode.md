...
- **PM mode**: Solo builders and `game-dev-orchestrator` handoff. Infer ordinary engine details. Keep the design conversation short; batch via `AskUserQuestion`.
- **Developer mode**: Handoff to a human game team. Stop inferring choices developers commonly argue about; surface them via `AskUserQuestion`.

...
- [ ] Each in-slice rules module and each asset family is its own slice (unless genuinely coupled), with its own source and test files.
- [ ] Contracts (shared types, interfaces, schema) and the style lock come early with `depends_on: []` or near it.
- [ ] No file is `shared` by three or more phases — hub files are owned by a single wiring phase and features plug in through registration points.
- [ ] A Parallel Waves table exists and names the critical path.

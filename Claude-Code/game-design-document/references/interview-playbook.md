...
- Default to 1 focused `AskUserQuestion` question per call; batch up to 3 only when tightly related.
- Use `multiSelect: true` for pillars, content types, platforms, juice priorities, cut lists.
...
After a major phase, prose recap then an `AskUserQuestion` call shaped like:
...
multiSelect: false
options:
  - label: "Yes — continue"        description: "Move to the next phase."
  - label: "Mostly right"          description: "Small fixes; tell me what to adjust."
  - label: "Needs rework"          description: "Something core is off; revisit this phase."
```

## Example Card

A good decision card teaches the design space in its descriptions:

```
question: "When the player dies mid-run, what carries over?"
header: "Failure"
multiSelect: false
options:
  - label: "Fail-forward meta (Recommended)"
    description: "Keep a currency that buys permanent unlocks (Hades-like). Softens death; needs a meta economy."
  - label: "Knowledge only"
    description: "Nothing persists but what the player learned (Spelunky-like). Pure skill; harsher for casual players."
  - label: "Checkpoint restore"
    description: "Respawn at the last checkpoint with progress intact. Lowest tension; favors story over mastery."
```
...
When the user says not to be interviewed at all ("just write it", "make reasonable calls", "I'm not around to answer"):

- Extract every decision already present in their brief — those are *not* assumptions.
- Fill the remaining gaps with the choice you would have marked "(Recommended)", and tag each one inline as **Assumption** so the user can find and overturn it later.
- Still apply the full quality bar: counts, verbs, rules, failure states, slice, cut-first. Speed changes interview depth, not document quality.
- Put the most consequential assumptions at the top of `08-open-questions.md` (or the lean equivalent) as "confirm or change" items.

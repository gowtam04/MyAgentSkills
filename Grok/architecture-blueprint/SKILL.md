...
description: >
  Create technical architecture documentation and implementation blueprints from product requirements using ask_user_question for decisions.
  Use when the user has requirements, a PRD, product discovery docs, or a feature/app idea and asks "design the architecture", "how should we build this", "technical design", "architect this", "document the implementation plan", "system design", or wants to move from requirements toward coding.
  This skill decides data model, component boundaries, APIs, file ownership, deployment shape (budget-constrained), technical tradeoffs, and granular TDD-ready build phases.
  It should run before /build-orchestrator for non-trivial apps or features. It writes architecture docs only and does not implement application code.
  For large or ambiguous scopes, the skill may enter Grok plan mode (enter_plan_mode) to produce a documentation-only plan before writing files.
...
## Plan Mode Output Contract (for the skill itself)

When the design task has genuine architectural ambiguity (multiple reasonable topologies, major tradeoffs with high reversal cost, unclear scope, etc.), call `enter_plan_mode` early. The final plan presented via `exit_plan_mode` must be a **documentation plan only**:

- The proposed plan must describe only how you will create or update architecture docs (sections, decisions to resolve, templates to use, output paths).
- The plan must explicitly state that no application source files, scaffolding, dependencies, tests, build configs, migrations, or implementation artifacts will be created.
- The plan may include implementation phases as *content inside* the architecture docs, but it must not execute those phases.
...
Present 2-4 realistic choices with clear implications in the option descriptions. Do not pad with bad options. If requirements contradict each other or conflict with constraints, stop and ask the user how to resolve via `ask_user_question`.
...
For greenfield, recommend a stack, explain why, and ask for approval when consequential. In PM mode, leave ordinary library choices to the builder (or `/build-orchestrator`). In Developer mode, surface package choices that affect code shape.
...
- Complete file structure with a purpose for every file (this is the ownership map for `/build-orchestrator`).
- Interface definitions at the seams (high detail for complex/security-sensitive/non-standard areas).
- Granular implementation phases (more smaller phases preferred). Each phase: What gets built, Depends on, Produces, Parallel opportunities, Test focus. In Developer mode also Success criteria and Review checklist / test split.
...
"Architecture complete. The next step is coordinated implementation — run `/build-orchestrator` (point it at the architecture docs or let it auto-discover them)."

## Special Notes for Grok Context

- When scope is large or genuinely ambiguous about approach (e.g. topology, major tradeoffs), offer to call `enter_plan_mode` yourself so the user can review a documentation-only plan before any files are written.

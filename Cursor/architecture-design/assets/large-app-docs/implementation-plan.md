...
This file is the primary task graph for `build-team`.
...
Machine-readable appendix for `build-team`. **Prose phases and File Structure above are the source of truth.** Generate this last; on conflict, fix the prose first. Required for multi-phase builds.
...
- **Mode** and **Budget Tier** from `overview.md` control TDD rigor in `build-team` (PM/small → lighter cycle; Developer/large → full red→review→impl→review→regression).
...
- Parallelism is capped by disjoint `owns` sets (not by wishful concurrency). Clear partitions enable 3–5 isolated implementers (`Task` `best-of-n-runner`) on large builds; soft ownership forces sequential work.
- Use `TodoWrite` for orchestrator state and a human-readable progress file for the user.
- After launching a worker wave, end the turn — do not poll `Task` agents.

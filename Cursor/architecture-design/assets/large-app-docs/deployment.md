...
Source of truth for `build-team` and the Build Manifest in `implementation-plan.md` — keep them identical. If a command is not known yet (greenfield scaffold), write the intended command and mark `TBD — set in scaffold phase`.

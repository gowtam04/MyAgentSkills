...
Shared expectations across `product-spec` → `architecture-design` → `build-team`.

## Stage boundaries

| Stage | Skill | Owns | Does not own |
|-------|--------|------|----------------|
| Discovery | `product-spec` | WHAT / WHY, personas, workflows, rules, testable ACs with stable IDs, constraints, assumptions | Frameworks, schemas, APIs, infra, code |
| Architecture | `architecture-design` | HOW: stack, data model, components, interfaces, phases, ownership map, requirement_refs, Build Manifest (multi-phase), deployment | Product intent inventing, application code |
| Build | `build-team` | Coordinated implementation from approved architecture | Redesigning architecture or product |
...
- Disjoint file ownership; serialize or isolate `shared` paths
- Use `Task` `best-of-n-runner` when ≥2 non-trivial implementers run in parallel (each gets a worktree + branch). Parent integrates before dependents.
- Parallelism capped by unblocked disjoint `owns` + context peak (1–2 small; 2–3 multi-phase shared workspace; 3–5 with isolation; up to 6 exceptional). TDD gates sequential per slice.
- After launching a worker wave: update `TodoWrite` + progress, then **end the turn**. Do not poll `Task` agents with `AwaitShell`.
...
- After discovery: next is skill `architecture-design`.
- After architecture: next is skill `build-team`.
